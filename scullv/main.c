...
struct scull_qset *scull_follow (struct scull_dev *dev, int n)
{
	...
	if (!qs)
	{
		qs = dev->data = vmalloc (sizeof (struct scull_qset));
		if (qs == NULL)
			return NULL;
	...
	while (n--)
	{
		if (! qs->next)
		{
			qs->next = vmalloc (sizeof (struct scull_qset));
			if (qs->next == NULL)
				return NULL;
			memset (qs, 0, sizeof (struct scull_qset));
		}
...
ssize_t scull_write (struct file *filp, const char __user *buf, size_t count, loff_t *f_pos)
{
	...
	if (!dptr->data)
	{
		dptr->data = vmalloc (qset * sizeof (char *));
		if (!dptr->data)
			goto out;
	...
	if (!dptr->data[s_pos])
	{
		dptr->data[s_pos] = vmalloc (quantum);
		if (!dptr->data[s_pos])
			goto out;
	}
...
int scull_trim (struct scull_dev *dev)
{
	...
	for (dptr = dev->data; dptr; dptr = next)
	{
		...
		{
			for (i = 0; i < qset; i++)
				vfree (dptr->data[i]); /* vfree(NULL) is OK */
			vfree (dptr->data);
			dptr->data = NULL;
		}
		next = dptr->next;
...
int scull_init_module (void)
{
	...
	scull_devices = vmalloc (scull_nr_devs * sizeof (struct scull_dev));
